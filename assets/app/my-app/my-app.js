...
var myApp = new Framework7({
	// Set to true to activate Material-specific App JS behavior
	material: true,
	...
	pushState: true,
	// If false then it will inherit View's animatePages parameter. If true, then push state navigation will happen without animation (page transitions)
	...
	preroute: function (view, options) {
		if(!loggedIn){
			...
			if(mustLogin.indexOf(dest) >= 0){
				myApp.modal({
					title:  'Informasi',
					text: 'Anda harus login terlebih dahulu untuk dapat menggunakan fitur ini.',
					buttons: [
						{
							text: 'BATAL',
							onClick: function() {
								if(view.activePage.name == 'intro'){		
									mainView.router.loadPage('app/home');
								}
							}
						},
						{
							text: '<b class="color-orange">LOGIN</b>',
							onClick: function() {
								myApp.closePanel();
								mainView.router.loadPage('user/pg/login');
							}
						},
					]
				})
...
myApp.onPageInit('*', function(page){
	if(loggedIn){
		$$('.login-class').removeClass('must-login');
	} else {
		$$('.login-class').addClass('must-login');
	}
})
/* ===== Home Page events  ===== */
...
/* ===== Iklan Page events  ===== */
myApp.onPageInit('iklan', function (page) {
	// Selengkapnya Button
	$$('.iklan-selengkapnya').on('click', function(){
		var height = $$('.iklan-deskripsi').height();
		if(height == 60){
			$$('.iklan-deskripsi').animate({'max-height':500}, { queue:false, duration:200 });
			$$('.iklan-selengkapnya').text('TUTUP');
		} else {
			$$('.iklan-deskripsi').animate({'max-height':60}, { queue:false, duration:200 })
			$$('.iklan-selengkapnya').text('SELENGKAPNYA');
		}
	})
	//	// Photo Gallery
	...
	//			if(i > 0 && i < $$('.swiper-slide').length - 1)
	//				photoBrowserPhotos[i - 1] = $$(this).attr("data-image");
	//		})
	...
	var actionSheetButtons = [
		[
			...
			{
				text: '<div class="action-sheet"><i class="icon material-icons">face</i> Facebook</div>',
				color: 'blue',
				onClick: function () {
					myApp.alert('He Hoou!');
				}
			...
			{
				text: '<div class="action-sheet"><i class="icon material-icons">photo_camera</i> Instagram</div>',
				color: 'orange',
				onClick: function () {
			...
			{
				text: '<div class="action-sheet"><i class="icon material-icons">turned_in</i> WhtasApp</div>',
				color: 'green',
				onClick: function () {
			...
			{
				text: '<div class="action-sheet"><i class="icon material-icons">sms</i> SMS</div>',
				color: 'teal',
				onClick: function () {
					myApp.alert('You have clicked red button!');
				}
			...
			{
				text: '<div class="action-sheet"><i class="icon material-icons">email</i> Email</div>',
				color: 'red',
				onClick: function () {
					myApp.alert('You have clicked red button!');
				}
			...
			{
				text: '<div class="action-sheet"><i class="icon material-icons">content_copy</i> Copy</div>',
				color: 'gray',
				onClick: function () {
					myApp.alert('You have clicked red button!');
				}
		...
		[
			{
				text: '<div class="action-sheet"><i class="icon material-icons">clear</i> Batal</div>',
			}
		]
	];
...
/* ===== Pengaturan Page events  ===== */
myApp.onPageInit('pengaturan', function (page) {
...
/* ===== Profil_Saya Page events  ===== */
myApp.onPageInit('profil_saya', function(page){
	var height = $$(window).height() - 105;
	$$('.profil-iklan').css('height', height + 'px');
})
/* ===== Profil_Penjual Page events  ===== */
myApp.onPageInit('profil_penjual', function(page){
	var height = $$(window).height() - 105;
	$$('.profil-iklan').css('height', height + 'px');
})
